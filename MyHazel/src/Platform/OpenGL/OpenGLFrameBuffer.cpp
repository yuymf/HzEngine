...
namespace Hazel {
		...
		: m_Specification(spec)
	{
		for (auto spec : m_Specification.Attachments.Attachments)
		{
			if (!Utils::IsDepthFormat(spec.TextureFormat))
				m_ColorAttachmentSpectifications.emplace_back(spec);
			else
				m_DepthAttachmentSpectification = spec;
		}
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		// Attachment
		if (m_ColorAttachmentSpectifications.size())
		{
			m_ColorAttachments.resize(m_ColorAttachmentSpectifications.size());
			...
			for (size_t i = 0; i < m_ColorAttachments.size(); i++)
			{
				...
				switch (m_ColorAttachmentSpectifications[i].TextureFormat)
				{
		...
		if (m_DepthAttachmentSpectification.TextureFormat != FramebufferTextureFormat::None)
		{
			Utils::CreateTextures(multisample, 1, &m_DepthAttachment);
			Utils::BindTexture(multisample, m_DepthAttachment);
			switch (m_DepthAttachmentSpectification.TextureFormat)
			{
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		auto& spec = m_ColorAttachmentSpectifications[attachmentIndex];
