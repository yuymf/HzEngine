...
namespace Hazel {
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			in.seekg(0, std::ios::end);         //pointer to the file end;
			result.resize(in.tellg());			//get the pointer,resize the result;
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		size_t pos = source.find(typeToken, 0);							//postion of "#"
		while (pos != std::string::npos)								//equals typeToken is in source!
		{
			size_t eol = source.find_first_of("\r\n", pos);				//find the "#type" end if exists;
			HZ_CORE_ASSERT(eol != std::string::npos, "Syntax error");
			size_t begin = pos + typeTokenLength + 1;					//postion of "v" or "f"
